...
import { Send, RotateCcw, User, Bot, TrendingUp } from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3 bg-gray-50">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-xs text-gray-300 mt-1">Escribe algo como: "Hola, quiero comprar un departamento"</p>
            </div>
          )}
        ...
        <div className="px-4 py-3 bg-white border-t border-gray-100">
          <div className="flex gap-2">
            ...
            <button onClick={send} disabled={!input.trim() || loading}
              className="p-2.5 bg-[#1a2744] text-white rounded-xl hover:bg-[#1a2744]/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      <div className="w-64 border-l border-gray-200 bg-white p-5 space-y-5 shrink-0 overflow-y-auto">
        ...
        {!meta ? (
          ...
          <>
            ...
            {/* Acción */}
            <div>
              <p className="text-xs font-medium text-gray-500 mb-1">Acción del agente</p>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium
                ...
                : meta.action === "escalate" ? "⚡ Escalar a Verónica"
                : meta.action === "continue" ? "💬 Continúa conversación"
                : meta.action}
              </span>
            </div>
                ...
                <div className="space-y-1.5">
                  {Object.entries(meta.collected).map(([k, v]) => v ? (
                    <div key={k} className="flex justify-between text-xs">
                      ...
                      <span className="text-gray-700 font-medium text-right max-w-[120px] truncate">
                        {String(v)}
                      </span>
                    </div>
                  ) : null)}
